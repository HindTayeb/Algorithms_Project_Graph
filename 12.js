...
function main_graph()   
{
	 ...
	 var graph = new Graph();
	 graph.label = "Exercise 9.2: 1b (Levitin, 3rd edition)";

	 graph.readGraph(_v, _e);
	 graph.printGraph();

	 document.write("<br>MST by Prim2 (PQ)<br>");
	 graph.prim2();
	 for (var i = 0; i < graph.Vt.length; i++)
	 {
		document.writeln("(", graph.Vt[i].parent,",",graph.Vt[i].vtree,")");
