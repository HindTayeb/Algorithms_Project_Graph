 ...
 * @author Hawazin aldosari
   @memberof #FNetwork
   this method print Network information
   (adjacent list,adjacent vertices)
*/
function printNetworkImpl()
{
    ...
    for (var i = 0; i < this.network.nv; i++)
    {
        ...
        {
			//print vertex with target vertex and edge weight ,edge flow 
...
{	
	var v = this.network.getVertex(i);//get vertex
	var adj = v.incidentEdge();//get incident edge for this vertex

	for(var i = 0; i < adj.length; i++)
	{
		if(adj[i].adjVert == j)
		{
			return adj[i].edgeFlow;//return edge flow for this vertex.
		}
	}
}
//---------------------------------------
...
function edgeCapImpl(i,j)
{
	...
	for(var i = 0; i < adj.length; i++)
	{
		if(adj[i].adjVert == j)
		{
			return adj[i].edgeWeight;//return capacity for edge
		}
	}
}
//-------------------------------------
