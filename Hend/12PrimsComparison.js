...
function main_graph()   
{
	 ...
	 g.makeGraph(5000, 3500000, true);
	 document.write(g.label);
	 console.time("Prim run time");
	 g.prim1();
	 console.timeEnd("Prim run time"); 
	 console.time("PQPrim run time");
	 g.PQprim2();
	 console.timeEnd("PQPrim run time");
	 console.time("HPQPrim run time");
	 g.HPQprim2();
	 console.timeEnd("HPQPrim run time"); 
}
...
// published docs section (ref. assignment page)
	...
	// student methods next (actual functions in student code sections)
	this.prim1 = primImpl;
	this.PQprim2 = PQprimImpl2
	this.HPQprim2 = HPQprimImpl2;
 ...
 */
 ...
 function primImpl()
 {
	...
	{//check the edges from current vertex
		...
		this.vert[nextVert.adjVert].visit = true;
	}
 }
		...
		//get and delete edge which has high priority in priority queue
		 edge_Min=PQ.deleteMin().item;
		    ...
		    //update edge tree (insert minimum edge which has vertex id and parent id)
	 	    this.Vt.push({parent:edge_Min.item.parent_i,vtree: edge_Min.item.vertex_i});
